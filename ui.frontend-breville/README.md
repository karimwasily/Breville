...
3. Run `npm run prepare` - Installed husky on parent folder and configured pre-commit hook for `ui.frontend-breville`.
...
```
2. Configure the `storybook/aem-cli` module in package.json (this has already been done in ui.frontent, use that as an example)
3. Create a `.storybook` folder and a `stories` folder
4. Add `main,js`, `middleware.js` and `preview.js` to the `.storybook` folder (use the ones already in ui.frontend)
5. Create an empty story for each component using `sba story`. This will create a page under /content/[brand-name]-style-system and a story for each component in ui.apps in the `stories` folder