...
public class BrevilleAnalyticsModel extends BaseModel {
	...
	public Map<String, Object> getGenericPageData() {
		...
		if (Objects.nonNull(brand)) {
			...
			genericPageData.put(AnalyticsConstants.BRAND_NAME_ANALYTICS, ApplicationConstants.BREVILLE_BRAND);
		}

		String contentPath = buildLinkPath(currentPage.getPath(), resourceResolver);
		log.info("contentPath :: {}", contentPath);

		// get the request path Info - as we dont get desired path in case of PLP/PDP
		// from currentPage object
		contentPath = request.getPathInfo();

		/** get the url */
		final String url = buildExternalizeUrl(request.getResource().getResourceResolver(),
				contentPath.replace(ApplicationConstants.BREVILLE_ROOT_PATH, ""), request.getResource());
		log.info("url :: {}", url);

		if (contentPath.contains(ApplicationConstants.BREVILLE_ROOT_PATH)) {
			contentPath = contentPath.replace(ApplicationConstants.BREVILLE_ROOT_PATH + ApplicationConstants.SLASH,
					StringUtils.EMPTY);
		} else {
		...
		final String siteContentPath = contentPath.contains(ApplicationConstants.LANGUAGE_MASTERS_NODE)
				? contentPath
						.replace(ApplicationConstants.LANGUAGE_MASTERS_NODE + AnalyticsConstants.PIPE_SEPARATOR,
								StringUtils.EMPTY)
						.replace(this.languageCode.toLowerCase() + AnalyticsConstants.PIPE_SEPARATOR, StringUtils.EMPTY)
				: contentPath
						.replace(this.countryCode.toLowerCase() + AnalyticsConstants.PIPE_SEPARATOR, StringUtils.EMPTY)
						.replace(this.languageCode.toLowerCase() + AnalyticsConstants.PIPE_SEPARATOR,
								StringUtils.EMPTY);
		...
		getSiteSectionData(genericPageData,
				siteContentPath.replace(ApplicationConstants.HTML_EXTENSION, StringUtils.EMPTY),
	...
	protected void getSiteSectionData(final Map<String, Object> genericPageData, final String siteContentPath,
		...
		if (content.length > ApplicationConstants.TWO
			...
			getSiteSection(genericPageData,
					contentPath.split(AnalyticsConstants.PIPE_OPERATOR)[ApplicationConstants.TWO], StringUtils.EMPTY,
					StringUtils.EMPTY, contentPath.split(AnalyticsConstants.PIPE_OPERATOR)[ApplicationConstants.TWO]);
